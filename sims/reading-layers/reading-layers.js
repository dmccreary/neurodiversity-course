...
let containerHeight = 470; // fixed height on page
...
function draw() {
    ...
    const baseWidth = max(containerWidth * 0.9, 400);
    ...
    for (let i = 0; i < levels.length; i++) {
        ...
        // the .9 is how fast each level gets smaller
        const currentWidth = baseWidth * (1 - (i / levels.length)*.9);
        ...
        // Draw level box
        rect(currentX, currentY-sectHeight, currentWidth, sectHeight-2, 10);
    ...
    text("Five Layers of Reading Comprehension", containerWidth/2, 30);
  
  

    const descriptionY = min(triangleHeight + 110, containerHeight - 40);
